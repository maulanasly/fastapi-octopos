...
class _PurchasingScreenState extends ConsumerState<PurchasingScreen> {
          ...
          if (order.status == 'draft') ...[
            TextButton(
              onPressed: () {
                ...
                _confirmMarkOrdered(order);
              },
              child: Text(s.of('markOrdered')),
            ),
            TextButton(
              onPressed: () {
                Navigator.of(ctx).pop();
  ...
  Future<void> _confirmMarkOrdered(PurchaseOrder order) async {
    final s = ref.read(stringsProvider);
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(s.of('markOrdered')),
        content: Text(s.of('confirmMarkOrdered')),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(false),
            child: Text(s.of('cancel')),
    ...
    if (ok != true) return;
    try {
      await ref.read(purchasingRepositoryProvider).markOrdered(order.id);
      if (mounted) setState(_reload);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
  ...
  Future<void> _createInvoice(PurchaseOrder order) async {
    ...
    for (final item in order.items) {
      if (item.quantityReceived > 0) {
        items.add({
          'purchase_order_item_id': item.id,
          'billed_quantity': item.quantityReceived,
          'billed_unit_cost': item.unitCost,
        });
      }
    }
    if (items.isEmpty) {
          ...
          child: Column(
            ...
            children: [
              Text(
                ...
                '${order.items.where((i) => i.quantityReceived > 0).length} lines',
              ),
              TextField(
    ...
    return Column(
      children: [
        _statusChips(
          ...
          statuses: const ['draft', 'ordered', 'partially_received', 'received', 'cancelled'],
